...
export default function UnifiedLoginPage() {
  const [portal, setPortal] = useState<"fan" | "volunteer" | "ops" | "admin">(
    "fan"
  );
    ...
    <div className="w-full max-w-md p-8 bg-[#1d2022] rounded-3xl shadow-2xl border border-[#00f2ff]/30 space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-xs text-[#b9cacb] font-mono">
          Select role to authenticate into StadiumPulse AI
        </p>
      </div>
      ...
      <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
        <div>
          <label
            htmlFor="auth-identity"
            className="block text-[10px] font-mono font-bold uppercase tracking-wider mb-1 text-[#b9cacb]"
          >
            {portal === "fan" ? "Mobile Number" : "Staff Email / Username"}
          </label>
          <div className="relative">
            {portal === "fan" ? (
              <Smartphone className="absolute left-3.5 top-3.5 w-4 h-4 text-[#00f2ff]" />
            ) : (
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <label
              ...
              className="block text-[10px] font-mono font-bold uppercase tracking-wider text-[#b9cacb]"
            >
              Passcode / Auth Token
            </label>
            <Link
      ...
      <div className="text-center">
        <Link
          href="/verify"
          className="text-xs font-mono text-[#b9cacb] hover:text-[#00f2ff]"
        >
