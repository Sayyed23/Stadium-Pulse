      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#1d2022] border border-[#00f2ff]/30 p-5 rounded-2xl shadow-2xl">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-mono font-bold text-[#5cf968] tracking-wider uppercase">
              Live Command Telemetry Stream
            </span>
          </div>
          <h2 className="text-2xl font-extrabold text-white tracking-tight">
            Control Room Master Console
          </h2>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            className="px-4 py-2 rounded-xl bg-red-500/20 text-red-400 border border-red-500/40 font-mono font-bold text-xs hover:bg-red-500/30 transition-all uppercase"
          >
            Emergency Lockdown
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 bg-[#1d2022] border border-[#3a494b]/40 rounded-2xl p-5 h-[620px] flex flex-col shadow-2xl">
          <h3 className="font-mono font-bold text-xs text-[#00f2ff] border-b border-[#3a494b]/30 pb-3 mb-4 flex items-center justify-between uppercase tracking-wider">
            <span className="flex items-center gap-2">
              <Users size={18} /> Zone Density & Flow Telemetry
            </span>
            <span className="text-[10px] text-[#b9cacb] font-normal">
              {zoneList.length} Sectors Active
            </span>
          </h3>
              ...
              <div className="grid grid-cols-2 gap-3">
                {zoneList.map((zone) => {
                  ...
                  return (
                    <div
                      key={zone.zone_id}
                      className={`p-4 rounded-xl border ${borderClass} bg-[#101415] relative overflow-hidden space-y-2`}
                    >
                      <div className="relative z-10">
                        <div className="font-bold text-xs text-white truncate">
                          {zone.zone_name}
                        </div>
                        <div className="text-3xl font-extrabold font-mono text-[#00f2ff] my-1">
                          {pct}%
                        </div>
                      ...
                      <div
                        className={`absolute bottom-0 left-0 h-1.5 transition-all duration-1000 ${colorClass}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  );
        ...
        <div className="lg:col-span-6 bg-[#1d2022] border border-[#3a494b]/40 rounded-2xl p-5 h-[620px] flex flex-col shadow-2xl">
          <h3 className="font-mono font-bold text-xs text-[#00f2ff] border-b border-[#3a494b]/30 pb-3 mb-4 flex items-center justify-between uppercase tracking-wider">
            ...
            <span className="text-[10px] text-[#5cf968] font-normal">
