    ...
    <div className="flex flex-col h-[calc(100vh-6rem)]">
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <ChatWindow role="ops" apiEndpoint="/api/copilot" onResponse={handleResponse} />
        </div>
        ...
        <div className="flex-1 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-6 overflow-y-auto flex flex-col">
          <h3 className="text-xl font-semibold mb-4 border-b border-zinc-200 dark:border-zinc-800 pb-2">Draft Ticket</h3>
          
          {draft ? (
            <div className="space-y-6 flex-1">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-zinc-50 dark:bg-zinc-950 p-3 rounded-md border border-zinc-200 dark:border-zinc-800">
                  <div className="text-xs text-zinc-500 uppercase font-semibold mb-1">Category</div>
                  <div className="font-medium capitalize">{draft.draft_incident.category.replace("_", " ")}</div>
                </div>
                <div className="bg-zinc-50 dark:bg-zinc-950 p-3 rounded-md border border-zinc-200 dark:border-zinc-800">
                  <div className="text-xs text-zinc-500 uppercase font-semibold mb-1">Priority</div>
                  <div className="font-medium capitalize flex items-center gap-2">
                    {draft.draft_incident.priority === "high" || draft.draft_incident.priority === "critical" ? <AlertTriangle size={16} className="text-red-500" /> : null}
              ...
              <div className="bg-zinc-50 dark:bg-zinc-950 p-3 rounded-md border border-zinc-200 dark:border-zinc-800">
                <div className="text-xs text-zinc-500 uppercase font-semibold mb-1 flex items-center gap-1"><MapPin size={14} /> Location</div>
                <div className="font-medium">{draft.draft_incident.zone_id || "Unknown"}</div>
              </div>

              <div className="bg-zinc-50 dark:bg-zinc-950 p-3 rounded-md border border-zinc-200 dark:border-zinc-800">
                <div className="text-xs text-zinc-500 uppercase font-semibold mb-1">Description</div>
              ...
              {draft.suggested_volunteer && (
                <div className="border border-blue-200 dark:border-blue-900 bg-blue-50 dark:bg-blue-900/20 p-4 rounded-md">
                  ...
                  <div className="text-sm space-y-1 mb-3">
                    <p><span className="font-medium text-zinc-600 dark:text-zinc-400">Volunteer:</span> {draft.suggested_volunteer.name}</p>
                    <p><span className="font-medium text-zinc-600 dark:text-zinc-400">Language:</span> {draft.suggested_volunteer.language.toUpperCase()}</p>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-zinc-400 dark:text-zinc-600 text-center space-y-4">
              ...
              <p className="max-w-[200px]">Describe the incident on the left to generate a structured ticket draft.</p>
            </div>
          )}
        </div>
      </div>
